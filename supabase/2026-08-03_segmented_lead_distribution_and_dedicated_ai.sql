-- Regras configuraveis de distribuicao por campanha/conjunto e WhatsApp
-- exclusivo da IA. Os defaults preservam integralmente as configuracoes atuais.
alter table public.corretoras
  add column if not exists distribuicao_regras jsonb not null default '[]'::jsonb;

alter table public.corretora_ai_configs
  add column if not exists sender_mode text not null default 'profile',
  add column if not exists dedicated_instance_name text;

alter table public.corretora_ai_configs
  drop constraint if exists corretora_ai_configs_sender_mode_check;
alter table public.corretora_ai_configs
  add constraint corretora_ai_configs_sender_mode_check
  check (sender_mode in ('profile', 'dedicated'));

create unique index if not exists corretora_ai_configs_dedicated_instance_uidx
  on public.corretora_ai_configs (dedicated_instance_name)
  where dedicated_instance_name is not null;

-- Configuracao inicial solicitada para a Facilita. Os ids sao resolvidos pelos
-- nomes atuais, portanto nenhum id de usuario fica fixo no codigo.
with facilita as (
  select cr.id, cr.nome
  from public.corretoras cr
  where lower(cr.nome) like '%facilita%'
  order by cr.created_at asc
  limit 1
), membros as (
  select p.id, lower(coalesce(p.nome, '')) as nome
  from public.profiles p
  join public.corretores c on c.id = p.corretor_id
  join facilita f on lower(trim(c.nome_empresa)) = lower(trim(f.nome))
  where p.status in ('active', 'ativo', 'Ativo')
), regras as (
  select jsonb_build_array(
    jsonb_build_object(
      'id', 'individual',
      'nome', 'Individual / PF',
      'termos', jsonb_build_array('medsenior', 'bestsenior'),
      'fallback', false,
      'membros', coalesce((
        select jsonb_agg(jsonb_build_object('profile_id', id, 'peso', 1) order by nome)
        from membros where nome like '%isabella%' or nome like '%isabela%' or nome like '%taina%'
      ), '[]'::jsonb)
    ),
    jsonb_build_object(
      'id', 'pme',
      ...
      'membros', coalesce((
        select jsonb_agg(jsonb_build_object(
          'profile_id', id,
          'peso', case when nome like '%camila%' then 2 else 1 end
        ) order by nome)
        from membros where nome like '%camila%' or nome like '%karen%' or nome like '%karren%'
      ), '[]'::jsonb)
    )
  ) as value
)
update public.corretoras cr
set distribuicao_regras = regras.value
from facilita, regras
where cr.id = facilita.id
  and (cr.distribuicao_regras is null or cr.distribuicao_regras = '[]'::jsonb);
