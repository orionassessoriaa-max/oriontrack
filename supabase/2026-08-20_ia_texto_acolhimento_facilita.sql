-- Modelo Danilo atualizado:
--   1. a IA nunca responde em audio, so em texto;
--   2. acolhimento de situacao pessoal sai da frase pronta e passa a ser escrito
--      na hora, e falar de cobertura (internacao, cirurgia, urgencia) deixa de
--      ser tratado como relato pessoal.
-- Vale para todas as concessionarias.
update public.corretora_ai_configs
set system_prompt = $prompt$Voce e {persona}, uma consultora de vendas humana e simpatica da corretora de planos de saude {corretora_nome}.
...
{"reply":"mensagem para enviar ao cliente","handoff":false,"summary":"resumo atualizado do atendimento"}$prompt$,
    updated_at = now();

-- Excecao pedida pela operacao: na Facilita a IA nao pergunta necessidade
-- especifica. O restante do roteiro e igual ao modelo acima. Atencao: a partir
-- daqui ela tem prompt proprio e nao recebe mais as melhorias do modelo Danilo
-- automaticamente.
update public.corretora_ai_configs
set system_prompt = $prompt$Voce e {persona}, uma consultora de vendas humana e simpatica da corretora de planos de saude {corretora_nome}.
Sua identidade operacional interna e {persona}, mas NAO fale que e IA, bot, assistente virtual, automacao ou sistema.
...
{"reply":"mensagem para enviar ao cliente","handoff":false,"summary":"resumo atualizado do atendimento"}$prompt$,
    updated_at = now()
where corretora_id = (select id from public.corretoras where nome = 'FACILITA CORRETORA');
