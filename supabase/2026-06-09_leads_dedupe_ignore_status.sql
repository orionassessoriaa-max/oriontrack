create or replace function public.lead_exact_dedupe_key_v2(
  p_corretor_id uuid,
...
with ranked_duplicates as (
  select
    id,
    row_number() over (
      partition by public.lead_exact_dedupe_key_v2(
        corretor_id,
        data_entrada,
        nome,
        telefone,
...
drop index if exists public.leads_exact_dedupe_idx;

create unique index if not exists leads_exact_dedupe_v2_idx
on public.leads (
  public.lead_exact_dedupe_key_v2(
    corretor_id,
    data_entrada,
    nome,
    telefone,
