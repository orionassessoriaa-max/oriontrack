...
async function elevenLabsTextToSpeechBase64(text: string) {
  ...
  const response = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=${outputFormat}`, {
    ...
    body: JSON.stringify({
      ...
      voice_settings: {
        stability: Number(process.env.ORION_LEAD_AI_ELEVEN_STABILITY || 0.35),
        similarity_boost: Number(process.env.ORION_LEAD_AI_ELEVEN_SIMILARITY || 0.85),
        style: Number(process.env.ORION_LEAD_AI_ELEVEN_STYLE || 0.45),
