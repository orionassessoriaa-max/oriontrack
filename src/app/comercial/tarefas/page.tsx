...
import { CalendarClock, Check, CheckCircle2, Circle, Clock3, Plus, RefreshCw, UserRound, X } from 'lucide-react';
...
export default function CommercialTasksPage() {
  ...
  async function create(event: React.FormEvent) { event.preventDefault(); setSaving(true); try { await api('/api/comercial/tasks', { method: 'POST', body: JSON.stringify({ ...form, vencimento: form.vencimento ? new Date(form.vencimento).toISOString() : null }) }); setOpen(false); setForm({ titulo: '', descricao: '', vencimento: '', prioridade: 'normal', responsavel_id: currentProfileId || '', lead_id: '' }); await load(); } finally { setSaving(false); } }
