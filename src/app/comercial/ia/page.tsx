...
import { Bot, Check, Clipboard, Clock3, MessageSquareText, Power, RefreshCw, Save, Settings, Sparkles } from 'lucide-react';
...
export default function CommercialAiPage() {
  ...
  return <div>
    ...
    <section className="kh-panel kh-sdr-config">
      ...
      <div className="kh-sdr-config-body"><div className="kh-sdr-status"><div className="kh-ai-mark"><Bot size={23} /></div><div><strong>IA SDR da Orion</strong><p>Use o prompt abaixo para ajustar o comportamento da Aline. As alterações valem para esta operação.</p></div><button type="button" className={`kh-toggle ${active ? 'active' : ''}`} onClick={() => setActive((value) => !value)} aria-pressed={active}><span /></button></div><label className="kh-sdr-prompt"><span><Settings size={14} /> Prompt da IA SDR</span><textarea className="kh-textarea" value={prompt} onChange={(event) => setPrompt(event.target.value)} disabled={role !== 'coordenador'} /><small>{role === 'coordenador' ? 'Somente o coordenador pode alterar o prompt.' : 'Apenas o coordenador pode alterar o prompt.'}</small></label><footer className="kh-sdr-config-footer">{notice && <span>{notice}</span>}{role === 'coordenador' && <button className="kh-button primary" onClick={() => void saveConfig()} disabled={saving}><Save size={15} /> {saving ? 'Salvando...' : 'Salvar prompt'}</button>}</footer></div>
    </section>
