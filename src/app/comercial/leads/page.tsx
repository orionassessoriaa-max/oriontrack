...
export default function CommercialLeadsPage() {
  ...
  const columnCount = canViewCommercialFinancials ? 16 : 13;
  ...
  function exportCsv() {
    const headers = ['DATA', 'NOME', 'TELEFONE', 'EMAIL', ...(canViewCommercialFinancials ? ['JA INVESTIU EM TRAFEGO?', 'FATURAMENTO MENSAL'] : []), 'PRIORIDADE', ...(canViewCommercialFinancials ? ['INVESTIMENTO'] : []), 'VIDAS', 'UTM SOURCE', 'UTM MEDIUM', 'UTM CAMPAIGN', 'UTM TERM', 'UTM CONTENT'];
    const rows = visible.map((lead) => [formatDate(lead.data_entrada), lead.nome, lead.telefone || '', lead.email || '', ...(canViewCommercialFinancials ? [lead.ja_investiu_trafego || '', lead.faturamento_mensal || ''] : []), lead.prioridade || '', ...(canViewCommercialFinancials ? [lead.investimento || ''] : []), lead.vidas || '', lead.utm_source || '', lead.utm_medium || '', lead.utm_campaign || '', lead.utm_term || '', lead.utm_content || '']);
