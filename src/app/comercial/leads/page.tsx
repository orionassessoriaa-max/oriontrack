...
export default function CommercialLeadsPage() {
  const { api, members, canViewCommercialFinancials, canViewCommercialLeadQualification } = useCommercial();
  ...
  const columnCount = canViewCommercialLeadQualification ? 16 : 13;
  ...
  function exportCsv() {
    const headers = ['DATA', 'NOME', 'TELEFONE', 'EMAIL', ...(canViewCommercialLeadQualification ? ['JA INVESTIU EM TRAFEGO?', 'FATURAMENTO MENSAL'] : []), 'PRIORIDADE', ...(canViewCommercialLeadQualification ? ['INVESTIMENTO'] : []), 'VIDAS', 'UTM SOURCE', 'UTM MEDIUM', 'UTM CAMPAIGN', 'UTM TERM', 'UTM CONTENT'];
    const rows = visible.map((lead) => [formatDate(lead.data_entrada), lead.nome, lead.telefone || '', lead.email || '', ...(canViewCommercialLeadQualification ? [lead.ja_investiu_trafego || '', lead.faturamento_mensal || ''] : []), lead.prioridade || '', ...(canViewCommercialLeadQualification ? [lead.investimento || ''] : []), lead.vidas || '', lead.utm_source || '', lead.utm_medium || '', lead.utm_campaign || '', lead.utm_term || '', lead.utm_content || '']);
  ...
  return <div>
    ...
    <section className="kh-sheet-wrap"><table className="kh-sheet-table"><thead><tr><th className="select"><button onClick={toggleAll} aria-label="Selecionar todos">{selected.size === visible.length && visible.length ? <CheckSquare2 size={16} /> : <Square size={16} />}</button></th><th>#</th><th>DATA</th><th>NOME</th><th>TELEFONE</th><th>EMAIL</th>{canViewCommercialLeadQualification && <><th>JA INVESTIU EM TRAFEGO?</th><th>FATURAMENTO MENSAL</th></>}<th>PRIORIDADE</th>{canViewCommercialLeadQualification && <th>INVESTIMENTO</th>}<th>VIDAS</th><th>UTM SOURCE</th><th>UTM MEDIUM</th><th>UTM CAMPAIGN</th><th>UTM TERM</th><th>UTM CONTENT</th></tr></thead><tbody>{visible.map((lead, index) => <tr key={lead.id} className={selected.has(lead.id) ? 'selected' : ''}><td className="select"><button onClick={() => toggle(lead.id)} aria-label={`Selecionar ${lead.nome}`}>{selected.has(lead.id) ? <CheckSquare2 size={15} /> : <Square size={15} />}</button></td><td>{index + 1}</td><td>{formatDate(lead.data_entrada)}</td><td className="name">{lead.nome}</td><td>{lead.telefone || '-'}</td><td>{lead.email || '-'}</td>{canViewCommercialLeadQualification && <><td>{lead.ja_investiu_trafego || '-'}</td><td>{lead.faturamento_mensal || '-'}</td></>}<td>{lead.prioridade || '-'}</td>{canViewCommercialLeadQualification && <td>{lead.investimento || '-'}</td>}<td>{lead.vidas || '-'}</td><td>{lead.utm_source || '-'}</td><td>{lead.utm_medium || '-'}</td><td>{lead.utm_campaign || '-'}</td><td>{lead.utm_term || '-'}</td><td>{lead.utm_content || '-'}</td></tr>)}{!visible.length && <tr><td colSpan={columnCount} className="kh-table-empty">{loading ? 'Carregando leads...' : 'Nenhum lead encontrado com esses filtros.'}</td></tr>}</tbody></table></section>
    {selected.size > 0 && <div className="kh-bulk-bar"><strong>{selected.size} selecionado{selected.size > 1 ? 's' : ''}</strong><select className="kh-select" defaultValue="" onChange={(event) => { if (event.target.value) void bulkStatus(event.target.value); }}><option value="" disabled>Mover para status...</option>{COMMERCIAL_STATUSES.map((item) => <option key={item}>{item}</option>)}</select><button type="button" className="kh-bulk-edit" disabled={selected.size !== 1} title={selected.size === 1 ? 'Editar lead selecionado' : 'Selecione apenas um lead para editar'} onClick={editSelected}><Edit3 size={15} /> Editar lead</button><button type="button" className="danger" disabled={actionLoading} onClick={() => void deleteSelected()}><Trash2 size={15} /> Excluir lead{selected.size > 1 ? 's' : ''}</button><button type="button" aria-label="Limpar selecao" onClick={() => setSelected(new Set())}><X size={17} /></button></div>}
    ...
    <CommercialLeadModal open={modalOpen} members={members} canViewFinancials={canViewCommercialFinancials} canViewQualification={canViewCommercialLeadQualification} lead={editing} onClose={() => { setModalOpen(false); setEditing(null); }} onSave={async (data) => { if (editing) await updateLead(editing.id, data); else await api('/api/comercial/leads', { method: 'POST', body: JSON.stringify(data) }); await load(); }} />
  </div>;
}
