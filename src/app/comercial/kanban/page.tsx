...
import { CalendarClock, CalendarDays, CalendarPlus, ChevronDown, ChevronUp, GripVertical, MessageSquare, Paperclip, Pencil, Phone, Plus, RefreshCw, Search, UserRound, X } from 'lucide-react';
...
type LeadInteraction = { id: string; comentario: string | null; anexo_url: string | null; anexo_nome: string | null; autor_nome: string; tipo?: string; metadata?: Record<string, unknown>; created_at: string };
...
export default function CommercialKanbanPage() {
  ...
  const [editingStage, setEditingStage] = useState<CommercialStage | null>(null);
  ...
  const visible = useMemo(() => leads.filter((lead) => {
    ...
    const matchesSdr = sdrFilter === 'todos'
      || (sdrFilter === 'sem_responsavel' ? !lead.sdr_id : lead.sdr_id === sdrFilter);
  ...
  async function moveLead(id: string, status: string) {
    ...
    try {
      const payload = await api('/api/comercial/leads', { method: 'PATCH', body: JSON.stringify({ id, status }) });
      if (payload.lead) setLeads((current) => current.map((lead) => lead.id === id ? payload.lead : lead));
      if (expandedLeadId === id) {
        const timeline = await api(`/api/comercial/leads/${id}/interactions`);
        setInteractionsByLead((current) => ({ ...current, [id]: timeline.interactions || [] }));
      }
    } catch { await load(); } finally { setMovingId(null); }
  }
  ...
  async function renameStage(event: React.FormEvent) {
    ...
    const label = editingStageName.replace(/[<>]/g, '').replace(/\s+/g, ' ').trim();
    if (!label || label === editingStage.label) { setEditingStage(null); return; }
    setStageSaving(true); setStageError(null);
    try {
      const payload = await api('/api/comercial/stages', { method: 'PATCH', body: JSON.stringify({ old_id: editingStage.id, label }) });
      setStages(payload.stages || stages);
      await load();
      setEditingStage(null);
    } catch (error) {
      setStageError(error instanceof Error ? error.message : 'Nao foi possivel renomear a etapa.');
    } finally { setStageSaving(false); }
  }
  ...
  async function createLeadTask(event: React.FormEvent, lead: CommercialLead) {
    ...
    try {
      const payload = await api('/api/comercial/tasks', { method: 'POST', body: JSON.stringify({ ...taskForm, lead_id: lead.id }) });
      const scheduledAt = payload.task?.vencimento || (taskForm.vencimento ? new Date(taskForm.vencimento).toISOString() : null);
  ...
  return (
    <div className={`kh-kanban-page ${canViewCommercialFinancials ? '' : 'kh-hide-commercial-financials'}`}>
      <header className="kh-page-head"><div><div className="kh-eyebrow">Pipeline de vendas</div><h1>Kanban</h1><p>Acompanhe a passagem do SDR para o closer e o avanco de cada negociacao.</p></div><div className="kh-actions"><div className="kh-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar lead..." /></div>{role !== 'sdr' && <label className="kh-date-preset"><UserRound size={15} /><select value={sdrFilter} onChange={(event) => setSdrFilter(event.target.value)} aria-label="Filtrar por SDR"><option value="todos">Todos os SDRs</option><option value="sem_responsavel">Sem responsavel</option>{sdrMembers.map((member) => <option key={member.profile_id} value={member.profile_id}>{member.nome}</option>)}</select></label>}<label className="kh-date-preset"><CalendarDays size={15} /><select value={datePreset} onChange={(event) => changeDatePreset(event.target.value as DatePreset)} aria-label="Período"><option value="todos">Todo o período</option><option value="hoje">Hoje</option><option value="ontem">Ontem</option><option value="7dias">Últimos 7 dias</option><option value="30dias">Últimos 30 dias</option><option value="mes">Este mês</option><option value="personalizado">Personalizado</option></select></label>{datePreset === 'personalizado' && <><input className="kh-input kh-date-filter" type="date" value={dateStart} onChange={(event) => setDateStart(event.target.value)} aria-label="Data inicial" /><input className="kh-input kh-date-filter" type="date" value={dateEnd} onChange={(event) => setDateEnd(event.target.value)} aria-label="Data final" /></>}<button className="kh-icon-button" onClick={() => void load()} aria-label="Atualizar"><RefreshCw size={17} className={loading ? 'kh-spin' : ''} /></button><button className="kh-button primary" onClick={() => { setInitialStatus('Oportunidade'); setModalOpen(true); }}><Plus size={17} /> Novo lead</button></div></header>
      ...
      {role === 'coordenador' && editingStage && <form className="kh-stage-add" onSubmit={renameStage}><input autoFocus className="kh-input" value={editingStageName} onChange={(event) => setEditingStageName(event.target.value)} placeholder="Novo nome da etapa" maxLength={60} required /><button className="kh-button primary" disabled={stageSaving}>Salvar nome</button><button type="button" className="kh-button" onClick={() => setEditingStage(null)}>Cancelar</button></form>}
      ...
      <CommercialLeadDetailsModal lead={expandedLeadId ? leads.find((lead) => lead.id === expandedLeadId) || null : null} members={members} canViewFinancials={canViewCommercialFinancials} stages={stages} onMoveStage={(status) => { if (expandedLeadId) void moveLead(expandedLeadId, status); }} interactions={expandedLeadId ? interactionsByLead[expandedLeadId] || [] : []} interactionText={interactionText} interactionFile={interactionFile} interactionSaving={interactionSaving} interactionError={interactionError} taskForm={taskForm} taskSaving={Boolean(taskSaving)} onTaskChange={(field, value) => setTaskForm((current) => ({ ...current, [field]: value }))} onCreateTask={async (event) => { const lead = leads.find((item) => item.id === expandedLeadId); if (lead) await createLeadTask(event, lead); }} onInteractionTextChange={setInteractionText} onInteractionFileChange={setInteractionFile} onAddInteraction={(event) => { const lead = leads.find((item) => item.id === expandedLeadId); if (lead) void addInteraction(event, lead.id); }} onClose={() => setExpandedLeadId(null)} onSave={async (data) => { const payload = await api('/api/comercial/leads', { method: 'PATCH', body: JSON.stringify(data) }); if (payload.lead) setLeads((current) => current.map((lead) => lead.id === payload.lead.id ? payload.lead : lead)); }} />
      <CommercialLeadModal open={modalOpen} members={members} stages={stages} initialStatus={initialStatus} lead={null} canViewFinancials={canViewCommercialFinancials} onClose={() => setModalOpen(false)} onSave={async (data) => { await api('/api/comercial/leads', { method: 'POST', body: JSON.stringify(data) }); await load(); }} />
    </div>
  );
}
