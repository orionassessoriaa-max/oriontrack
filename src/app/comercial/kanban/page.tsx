...
export default function CommercialKanbanPage() {
  ...
  return (
    <div className="kh-kanban-page">
      ...
      <div className="kh-kanban" aria-label="Pipeline comercial">
        {COMMERCIAL_STATUSES.map((status, index) => {
          ...
          return <section key={status} className="kh-kanban-column" onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragging) void moveLead(dragging, status); setDragging(null); }}>
            <header style={{ '--stage-hue': `${205 + (index * 7) % 105}` } as React.CSSProperties}><div><span>{String(index + 1).padStart(2, '0')}</span><strong>{status}</strong><b>{statusLeads.length}</b></div><small>{currency(total)}</small></header>
      ...
      <CommercialLeadModal open={modalOpen} members={members} onClose={() => setModalOpen(false)} onSave={async (data) => { await api('/api/comercial/leads', { method: 'POST', body: JSON.stringify(data) }); await load(); }} />
    </div>
  );
}
