        ...
        <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => setTeamSummaryOpen((current) => !current)} aria-expanded={teamSummaryOpen} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-black uppercase tracking-widest text-slate-700 transition-all hover:bg-slate-100">
                <Users size={15} /> {teamSummaryOpen ? 'Ocultar equipe' : `Ver equipe (${teamMembers.length})`}
                <ChevronDown size={15} className={`transition-transform ${teamSummaryOpen ? 'rotate-180' : ''}`} />
              </button>
              {teamSummaryOpen && (
