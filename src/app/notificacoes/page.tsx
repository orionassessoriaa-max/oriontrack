  ...
  return (
    <InternalLayout>
      <div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-gray-900">Notificacoes</h1>
          <p className="font-medium text-gray-500">Avisos enviados pelo admin para corretores e gestores.</p>
        </div>
        <button
          onClick={fetchNotifications}
          className="flex w-fit items-center gap-2 rounded-2xl border border-gray-100 bg-white px-5 py-3 font-black text-gray-700 shadow-sm transition-colors hover:bg-gray-50"
        >
      ...
      {profile?.tipo_usuario === 'admin' && (
        <form onSubmit={sendNotification} className="mb-8 rounded-[2.5rem] border border-gray-100 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end">
            <div className="space-y-2 md:w-56">
              <label className="ml-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Enviar para</label>
              <select
            ...
            <div className="flex-1 space-y-2">
              <label className="ml-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Titulo</label>
              <input
                required
          ...
          <div className="divide-y divide-gray-50">
            {notifications.map((notification) => (
              <button
                ...
                className="block w-full p-6 text-left transition-colors hover:bg-slate-50"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="mb-2 flex items-center gap-2">
                      {!notification.lida && <span className="h-2.5 w-2.5 rounded-full bg-blue-600" />}
                      <h2 className="font-black text-gray-900">{notification.titulo}</h2>
                    </div>
                    <p className="max-w-3xl text-sm font-medium leading-relaxed text-gray-500">{notification.mensagem}</p>
                  </div>
