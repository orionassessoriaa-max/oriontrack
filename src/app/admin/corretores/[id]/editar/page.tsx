...
export default function EditarCorretorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const [corretorRes, gestoresRes, profileRes, brokeragesRes] = await Promise.all([
        supabase
          .from('corretores')
          .select('*')
          .eq('id', id)
      ...
      const data = corretorRes.data;
      if (data) {
        setFormData({
          nome: data.nome || '',
          email: data.email || '',
          telefone: data.telefone || '',
          link_pagina: data.link_pagina || '',
          status: data.status?.toLowerCase() === 'ativo' || data.status === 'active' ? 'ativo' : 'inativo',
      ...
      setError(message === 'JSON object requested, multiple (or no) rows returned' ? 'Corretor não encontrado.' : 'Erro ao carregar dados.');
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          time_operacional: formData.time_operacional,
          gestor_trafego_id: gestorTrafegoId || null
        })
        .eq('id', id);
      ...
      setError(err instanceof Error ? err.message : 'Erro ao atualizar dados do corretor.');
      setSaving(false);
    }
  };
  ...
  return (
    <InternalLayout>
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Editar Corretor</h1>
        <p className="text-gray-500 font-medium">Atualize os dados e a equipe Orion deste parceiro.</p>
      </div>
      ...
      <div className="max-w-4xl">
        ...
        {success && (
          <div className="mb-6 p-6 bg-green-50 border border-green-100 rounded-[2rem] text-green-700 font-black flex items-center gap-4 animate-in zoom-in-95">
            ...
            Corretor atualizado com sucesso! Redirecionando...
          </div>
        )}
