...
export async function GET(request: Request) {
  ...
  const { data: profiles } = profileIds.length
    ? await supabaseAdmin
        .from("profiles")
        .select(
          "id,nome,email,email_real,foto_url,status,corretor_id,equipe_orion",
        )
        .in("id", profileIds)
    : { data: [] };
  ...
  const members = (rows || [])
    .filter((row) => profileMap.has(row.profile_id))
    .map((row) => {
...
export async function POST(request: Request) {
  ...
  if (body.action === "create") {
    ...
    const emailReal = String(body.email || "")
      .trim()
      .toLowerCase();
    if (!nome || !["coordenador", "closer", "sdr"].includes(papel)) {
      return NextResponse.json(
        { error: "Nome e função comercial são obrigatórios." },
        { status: 400 },
      );
    }
    ...
    if (
      profileError &&
      /equipe_orion|schema cache/i.test(String(profileError.message || ""))
    ) {
      const profileWithoutTeam = Object.fromEntries(
        Object.entries(profilePayload).filter(
          ([key]) => key !== "equipe_orion",
        ),
      );
    ...
    return NextResponse.json({
      ok: true,
      credentials: { nome, email, senhaProvisoria, papel },
    });
  }
  ...
  if (!profileId || !["coordenador", "closer", "sdr"].includes(papel)) {
    return NextResponse.json(
      { error: "Perfil e funcao sao obrigatorios." },
      { status: 400 },
    );
  }
...
export async function PATCH(request: Request) {
  ...
  const papel = String(body.papel || "");
  if (!profileId)
    return NextResponse.json({ error: "Perfil obrigatorio." }, { status: 400 });
